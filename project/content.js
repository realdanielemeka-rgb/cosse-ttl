/*
 * content.js — single source of truth for the Cossé TTL prototype.
 *
 * In the Next.js productionisation this becomes `src/content.ts` (strongly typed).
 * Pages import this module and render from it; nothing is hardcoded in components.
 * All case studies are fictional but plausible for the Nigerian market.
 * Every metric is a PLACEHOLDER — see `note: 'placeholder'` on each impact stat.
 */

export const positioning = {
  eyebrow: "INTEGRATED MARKETING COMMUNICATIONS · LAGOS",
  // Hero manifesto — leads with the descriptor, recast as "total brand experiences".
  ...
  // The strategic note for the paper beat — never surfaced as body copy elsewhere.
  totalNote:
...
// The seven-part narrative spine — the agency's way of working, and the
// scaffold of every case study.
export const method = [
...
// Studio story for the About page (present-tense authority, no “dust”).
export const about = {
...
// The DNA, expressed as a value set (tone, not slogans).
export const values = [
...
// Thinking / POV index (placeholder essays — real articles slot in at production).
...
// Contact / Start-a-project surface.
...
// Studio — the integrated room, the craft and the place (distinct from About's story).
export const studio = {
  lead: "One room. Every discipline at the same table.",
  intro: "Strategy, creative, media, production and activation build together from day one — no silos to hand a brief across, no idea diluted in translation. That is what “total” means in practice.",
  ways: [
...
// Present-tense authority. Placeholder milestones flagged inline.
...
// Placeholder names/roles. Avatars are typographic monograms — no photos of
// real, identifiable people. Swap for real licensed portraits in production.
...
// Filter chips used on the Work index.
...
/*
 * Case studies. `discipline` is the primary tag (drives the Work filter);
 * `tags` are the disciplines shown on the card and case hero.
 * `spine` holds the seven-part story for the detail page.
 * `keyword` sets the big typographic thumbnail word.
 */
export const cases = [
  {
    ...
    spine: {
      ...
      idea:
        "Make every agent a doorway, not a device. Your money. Your town. Your hands.",
      execution: [
  ...
  },
  {
    ...
    spine: {
      ...
      idea:
        "Brewed for the bend. A stout for people who take the hard road on purpose.",
      execution: [
  ...
  },
  {
    ...
    spine: {
      ...
      idea:
        "Count me in. Bundles built around what people are working toward, not gigabytes.",
      execution: [
  ...
  },
  {
    ...
    spine: {
      ...
      idea:
        "The long season. We don't sell a good year — we help you survive the bad one.",
      execution: [
  ...
  },
  {
    ...
    spine: {
      ...
      idea:
        "Show up. A promise from the system to the street, kept in both directions.",
      execution: [
  ...
  },
  {
    ...
    spine: {
      ...
      idea:
        "Homecoming. Every transfer is an arrival.",
      execution: [
  ...
  },
  {
    ...
    spine: {
      ...
      idea:
        "Heirloom, not commodity. The recipe your grandmother trusted, made worthy of your shelf.",
      execution: [
  ...
  },
  {
    ...
    spine: {
      ...
      idea:
        "Lagos Loud. Don't sponsor the stage — be the reason it exists.",
      execution: [
  ...
  },
  {
    ...
    spine: {
      ...
      idea:
        "Hold steady. Patience is not the safe choice. It's the bold one.",
      execution: [
...
// Real client roster. Logos were sliced from the supplied client sheet onto
// uniform white tiles (clients/NN.png) so the colourful set reads consistently.
// In production swap each for the brand's official vector logo (white-knockout
// set preferred for the dark UI).
